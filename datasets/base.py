...
import numpy as np
import math
from .draw_gaussian import draw_umich_gaussian, gaussian_radius, draw_umich_gaussian_new,gaussian_radius_new,draw_umich_gaussian_new_angle
from .transforms import random_flip, load_affine_matrix, random_crop_info, ex_box_jaccard
from . import data_augment
import matplotlib.pyplot as plt

def imshow_heatmap(pr_dec, images_):
    cat = [     'plane',
                'baseball-diamond',
                'bridge',
                'ground-track-field',
    ...
    hm_ = pr_dec['hm']
    images_ = np.transpose((images_ + 0.5) * 255, (1, 2, 0)).astype(np.uint8)
    fig = plt.figure(1,dpi=300)
    for i in range(hm_.shape[0]):
        ax = fig.add_subplot(4, 4, i+1)
        ax.set_title(cat[i])
        ax.set_axis_off()
        ax.imshow(hm_[i])
    ax15 = fig.add_subplot(4, 4, 16)
    ax15.set_title('input image')
    ax15.imshow(images_)
    plt.axis('off')    
    plt.savefig('/workstation/fyy/heatmap/heatmap_new1.png')


# def imshow_heatmap(pr_dec, images_):
#     hm_ = pr_dec['hm'][0, :, :]
#     images_ = np.transpose((images_ + 0.5) * 255, (1, 2, 0)).astype(np.uint8)
#     hm_ = cv2.resize(hm_, (images_.shape[1], images_.shape[0]))
#     fig = plt.figure(1)
#     ax1 = fig.add_subplot(1, 2, 1)
#     ax1.set_title('center hm')
#     ax1.imshow(hm_)
#     ax1.set_axis_off()
...
#     plt.savefig('/workstation/fyy/heatmap/heatmap1.png')
...
class BaseDataset(data.Dataset):
    ...
    def generate_ground_truth(self, image, annotation):
        ...
        for k in range(num_objs):
            ...
            # long = max(bbox_h,bbox_w)
            # short = min(bbox_h,bbox_w)
            # lengthwise_ratio = (1. * short)/(1. * long)

            # if lengthwise_ratio >= 0.5 or short<12:
            #     heatmap_type = 'circle'
            # else:
            #     heatmap_type = 'ellipse'                
            ...
            #     rect_ = cv2.minAreaRect(pts_4)
            #     angle = rect_[2]

            #     # 调整角度范围为 [0, 180)
            #     adjusted_angle = angle if angle >= 0 else angle + 180

            #     # 判断宽度是否大于高度
            #     if rect_[1][0] > rect_[1][1]:
            #         # 宽度大于高度，无需调整角度
            #         angle_clockwise = adjusted_angle
            #         rh, rw = gaussian_radius_new((math.ceil(rect_[1][1]), math.ceil(rect_[1][0])))

            #     else:
            #         # 高度大于宽度，将角度加上 90 度
            #         angle_clockwise = adjusted_angle + 90
            #         rh, rw = gaussian_radius_new((math.ceil(rect_[1][0]), math.ceil(rect_[1][1])))

            #     rh = max(0.00001, int(rh))
            #     rw = max(0.00001, int(rw))
            #     ct = np.asarray([cen_x, cen_y], dtype=np.float32)
            #     ct_int = ct.astype(np.int32)
            #     draw_umich_gaussian_new_angle(hm[annotation['cat'][k]], ct_int, rh, rw, angle_clockwise)

            

            # #高斯热图（椭圆，无角度
            # rh, rw = gaussian_radius_new((math.ceil(bbox_h), math.ceil(bbox_w)))
            # rh = max(0,int(rh))
            # rw = max(0,int(rw))
            # ct = np.asarray([cen_x, cen_y], dtype=np.float32)
            # ct_int = ct.astype(np.int32)
            # draw_umich_gaussian_new(hm[annotation['cat'][k]], ct_int, rh, rw)    
