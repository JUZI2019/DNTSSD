...
class EvalModule(object):
    ...
    def evaluation(self, args, down_ratio):
        ...
        # result_path = os.path.join(args.save_dir, args.data_dir.split('/')[-1], 'evalTxt')
        result_path = os.path.join(args.save_dir, args.data_dir.split('/')[-2]+'_baseline', 'evalTxt') 
        ...
        if 'dota' in args.dataset:
            merge_path = os.path.join(args.save_dir, args.data_dir.split('/')[-2]+'_baseline', 'evalTxt_merge')
